...
const ScheduleView: React.FC = () => {
  ...
  const handleExportToCalendar = () => {
    ...
    classes.forEach(classSession => {
        ...
        const event = [
            'BEGIN:VEVENT',
            `UID:${classSession.id}@${siteSettings.academyName.replace(/\s/g, '')}.com`,
            ...
            `LOCATION:${siteSettings.academyName}`,
            'END:VEVENT'
        ].join('\r\n');
        icsString += `\r\n${event}`;
    });
            ...
            <div key={day}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {dayClasses.map(classSession => {
                  const googleCalendarLink = generateGoogleCalendarLink(classSession, siteSettings.academyName);
                  return (
